  ...
  <div class="sub" v-bind:style="{backgroundImage:'url('+ require('../../assets/mast/sub.jpg') +')'}">
    <div class="sub_form">
      ...
      <div v-for="v in mess" :key="v.id" v-show="v.sta">
        <div v-for="vv in v._data" :key="vv.id">
          <div class="sub_title_two">
            {{vv.name}}
          </div>
          <div class="sub_person">
            <ul class="sub_ul">
              <li class="sub_cate_item" v-for="vvv in vv._data" :key="vvv.id" @click="vvv.sta=!vvv.sta" :class="{sub_text:vvv.sta}">{{vvv.name}}</li>
            </ul>
          </div>
        </div>
      </div>
...
<script>
import {getallfrom, addsmess} from '../../assets/api/servers'
  ...
  methods: {
    subform () {
      ...
      for (let i in this.mess) {
        for (let j in this.mess[i]['_data']) {
          for (let z = 0; z < this.mess[i]['_data'][j]['_data'].length; z++) {
            // eslint-disable-next-line
            if (this.mess[i]['_data'][j]['_data'][z].sta > 0) {
              let cate = []
              cate.push(this.mess[i].cname)
              cate.push(this.mess[i]['_data'][j].name)
              cate.push(this.mess[i]['_data'][j]['_data'][z].name)
              this.data.cid.push(cate)
            }
          }
        }
      }
